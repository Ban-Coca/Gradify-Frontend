...
import { getStudentBreakdown } from "@/services/teacher/teacherService";
...
export default function StudentDetailsDialog({
  ...
  const { studentDetail, isLoading, error } = useStudentDetail(
    classId,
    propStudent?.id
  );
  ...
  const { data: calculatedGradeData, isLoading: isGradeLoading } = useQuery({
    ...
    queryFn: () =>
      getCalculatedGrade(propStudent?.id, classId, getAuthHeader()),
  ...
  const { data: courseTableData, isLoading: isCourseDataLoading } = useQuery({
    ...
    queryFn: () =>
      getStudentCourseTableData(propStudent?.id, classId, getAuthHeader()),
    enabled: !!propStudent?.id && !!classId,
  });

  const {data: studentBreakdown, isLoading: isBreakdownLoading} = useQuery({
    queryKey: ['studentBreakdown', propStudent?.id, classId],
    queryFn: () => getStudentBreakdown(classId, propStudent?.id, getAuthHeader()),
    enabled: !!propStudent?.id && !!classId
  })
  ...
  const processedStudent =
    studentDetail && calculatedGradeData
      ? {
          ...
          grade:
            convertCalculatedGradeToLetterGrade(calculatedGradeData) || "N/A",
          percentage: calculatedGradeData || 0,
          status: getStudentStatus(calculatedGradeData || 0),
          gradeHistory: studentBreakdown?.gradeBreakdown || [],
  ...
  if (isLoading || isGradeLoading || isCourseDataLoading || isBreakdownLoading) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent
          aria-describedby="report-details-description"
          className="w-full sm:max-w-4xl max-h-[90vh] flex flex-col"
        >
          <DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <CardContent>
                <div className="flex flex-col items-center mb-4">
                  ...
                  <Badge
                    className="mt-1"
                    variant={
                      student.status === "Excellent" ? "default" : "outline"
                    }
                  >
                    {student.status}
                  </Badge>
            ...
            <Card className="md:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Academic overview for student</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <TabsContent value="overview" className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4">
                      <div className="bg-muted rounded-lg p-3 text-center">
                        ...
                        <p className="text-xs text-white dark:text-muted-foreground">
                          Current Grade
                        </p>
                      </div>
                      <div className="bg-muted rounded-lg p-3 text-center">
                              ...
                              <div key={index} className="space-y-2 py-1">
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-semibold">
                                    {grade.score}/{grade.maxPoints} ({grade.weightedContribution.toFixed(2)}% of {grade.weight}%)
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <Progress
                                    value={(grade.score / grade.maxPoints) * 100}
            ...
            <Card className="md:col-span-3">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Recent feedback provided to the student
                  </CardDescription>
                </div>
                <Button>
                      ...
                      <Card key={report.reportId}>
                        <CardHeader className="pb-2">
                          <div className="flex justify-between items-start">
                            ...
                            <Badge
                              variant={
                                report.notificationType === "grade-alert"
                                  ? "destructive"
                                  : "default"
                              }
                            >
                        ...
                        <CardContent>
                          <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: DOMPurify.sanitize(report.message),
                            }}
                          />
                        </CardContent>
                      </Card>
