...
export default function NotificationsPage() {
  ...
  const formatNotifications = (apiNotifications) => {
    ...
    return apiNotifications.map((notification) => ({
      ...
      description: cleanNotificationText(
        notification.message || notification.description
      ),
      time: new Date(notification.date).toLocaleString(),
      type: notification.notificationType || "info",
      read: notification.read,
      icon: getIconForType(notification.notificationType || "info"),
          ...
          <TabsContent value="all" className="mt-4 space-y-4">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  View all your activity and system notifications
                </CardDescription>
              </CardHeader>
...
function NotificationSkeleton({ count = 3 }) {
  return Array(count)
    ...
    .map((_, index) => (
      <div
        key={index}
        className="flex items-start gap-4 p-4 border-b last:border-0 animate-pulse"
      >
...
function NotificationItem({
  ...
  return (
    <div
      className={`flex items-start gap-4 p-4 border-b last:border-0 cursor-pointer ${
        !read ? "bg-muted/50" : ""
      }`}
      onClick={() => navigate(`/student/feedback/${reportId}`)}
    >
      <div
